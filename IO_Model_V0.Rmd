...
FD Vector => growth shares code  
Biosolutions Shares - > Johanna Code (Unido)- > Output = Biosolution output totals. This also calculates the 2021-2023 using the growth shares
Employment Ratio Code: Takes as Input> Employment-shares Output = table with employment ratio per industry/country pair.  Then it also takes the IO Model Results as inputs to export employment effects => 2024, 2024H, 2025H
IO Model Code: uses Employment Ratios and Unido Data + code mapping files = > direct and indirect effects on output and employment → 2024, 2025L, 2025H
Graph code = > uses the IO Model excel outputs to generate charts
it makes sense to have one Io Model and then various outputs and inputs instead of 3 files.
